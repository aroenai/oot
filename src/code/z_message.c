...
void Message_DrawTextJPN(PlayState* play, Gfx** gfxP) {
    ...
    for (i = 0; i < msgCtx->textDrawPos; i++) {
        ...
        switch (character) {
            ...
            default:
                switch (character) {
                    case MESSAGE_WIDE_PARENTHESES_LEFT:
                    case MESSAGE_WIDE_KAGIKAKKO_LEFT:
                        msgCtx->textPosX -= 6;
                        break;
                    case MESSAGE_WIDE_NAKATEN:
                        msgCtx->textPosX -= 3;
                        break;
                    case MESSAGE_WIDE_QUESTION_MARK:
                    case MESSAGE_WIDE_EXCLAMATION_MARK:
                    case MESSAGE_WIDE_CIRCUMFLEX_ACCENT:
                    case MESSAGE_WIDE_ONE:
                        msgCtx->textPosX -= 2;
                        break;
                }
                ...
                switch (character) {
                    case MESSAGE_WIDE_PERIOD:
                        msgCtx->textPosX += 3;
                        break;
                    case MESSAGE_WIDE_PARENTHESES_RIGHT:
                    case MESSAGE_WIDE_KAGIKAKKO_RIGHT:
                        msgCtx->textPosX += 5;
                        break;
                    case MESSAGE_WIDE_TOUTEN:
                    case MESSAGE_WIDE_KUTEN:
                    case MESSAGE_WIDE_DOUBLE_QUOTATION_MARK_RIGHT:
                        msgCtx->textPosX += 7;
                        break;
                    case MESSAGE_WIDE_CIRCUMFLEX_ACCENT:
                    case MESSAGE_WIDE_NUMBER_SIGN:
                    case MESSAGE_WIDE_ASTERISK:
                        msgCtx->textPosX += 9;
                        break;
                    case MESSAGE_WIDE_NAKATEN:
                        msgCtx->textPosX += 10;
    ...
    if (gSaveContext.language == LANGUAGE_JPN && !sTextIsCredits) {
        while (true) {
            ...
            if (curCharWide == MESSAGE_WIDE_NAME) {
                ...
                for (i = 0; i < 4; i++) {
                    Font_LoadKanji(font, digits[i] + MESSAGE_WIDE_ZERO, charTexIdx);
                    charTexIdx += FONT_CHAR_TEX_SIZE;
                    msgCtx->msgBufDecodedWide[decodedBufPos] = digits[i] + MESSAGE_WIDE_ZERO;
                    decodedBufPos++;
                    if (i == 1) {
                        Font_LoadKanji(font, MESSAGE_WIDE_MINUTES, charTexIdx);
                        charTexIdx += FONT_CHAR_TEX_SIZE;
                        msgCtx->msgBufDecodedWide[decodedBufPos] = MESSAGE_WIDE_MINUTES;
                        decodedBufPos++;
                    } else if (i == 3) {
                        Font_LoadKanji(font, MESSAGE_WIDE_SECONDS, charTexIdx);
                        charTexIdx += FONT_CHAR_TEX_SIZE;
                        msgCtx->msgBufDecodedWide[decodedBufPos] = MESSAGE_WIDE_SECONDS;
                    }
                }
                ...
                for (i = 0; i < 4; i++) {
                    ...
                    decodedBufPos++;
                    if (i == 1) {
                        Font_LoadKanji(font, MESSAGE_WIDE_HOURS, charTexIdx);
                        charTexIdx += FONT_CHAR_TEX_SIZE;
                        msgCtx->msgBufDecodedWide[decodedBufPos] = MESSAGE_WIDE_HOURS;
                        decodedBufPos++;
                    } else if (i == 3) {
                        Font_LoadKanji(font, MESSAGE_WIDE_MINUTES, charTexIdx);
                        charTexIdx += FONT_CHAR_TEX_SIZE;
                ...
                if (curCharWide == MESSAGE_WIDE_FADE) {
                    ...
                    msgCtx->choiceNum = 3;
                    R_TEXT_INIT_XPOS += 32;
                } else if (curCharWide != MESSAGE_WIDE_SPACE) {
